...
Exception: `question_matching_items.id` and `question_ordering_items.id` are random version-4 UUIDs.
Students receive these identifiers, and time-ordered identifiers would reveal authoring order and
therefore the Matching pairs or the correct Ordering (Stage 8 Backend Phase 2 finding P1-1).
...
| `scheduled_at` | timestamptz(6) | yes | Optional preparation/schedule instant; microsecond precision preserved |
...
Stores shuffled left/right items without encoding the correct answer in display order or in
identifiers (`id` is a random version-4 UUID; see the UUID convention exception).
...
`id` is a random version-4 UUID so identifier order never encodes `correct_position`.
