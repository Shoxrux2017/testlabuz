...
class StudentBlitzFileAnswerApiTest extends TestCase
{
    ...
    public function test_each_real_binary_format_creates_one_private_pending_answer_and_resumes_only_safe_metadata(string $extension, string $mime): void
    {
        ...
        $response->assertExactJson(['data' => ['question_id' => $question->id, 'type' => 'file_based',
            'answer' => $canonical, 'updated_at' => '2026-09-17T12:00:00Z']]);
        ...
        $resume = $this->startStudentBlitz($student, $blitz->assessment)->assertOk()
            ->assertJsonPath('data.answers', [$response->json('data')]);
        foreach ([$response->json(), $resume->json('data.answers')] as $payload) {
            $this->assertNoFileAnswerSecrets($payload);
            $this->assertNoAnswerSecrets($payload);
        }
    ...
    {
        [$student, $blitz, $attempt] = $this->fileAnswerContext();
        $nonFile = $this->answerQuestion($blitz, 'true_false', 2);
        ...
        $otherAssessment = Assessment::factory()->blitz()->create(['institution_id' => $student->institution_id]);
    ...
    {
        $path = tempnam(sys_get_temp_dir(), 's08_be006_api_');
