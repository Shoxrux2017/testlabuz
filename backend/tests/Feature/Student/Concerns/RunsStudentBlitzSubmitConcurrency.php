...
trait RunsStudentBlitzSubmitConcurrency
{
    use RefreshDatabase;
    ...
    protected function setUp(): void
    {
        ...
        $this->storageRoot = storage_path('framework/testing/blitz-submit-concurrency-'.Str::uuid());
        config(['filesystems.disks.blitz_submit_concurrency' => [
            'driver' => 'local', 'root' => $this->storageRoot, 'visibility' => 'private', 'throw' => true,
        ]]);
        Storage::forgetDisk('blitz_submit_concurrency');
    ...
    protected function arguments(string $student = 'first', array $overrides = []): array
    {
        return array_replace($this->ids, [
            ...
            'key' => (string) Str::uuid(), 'observed_at' => '2026-09-17 09:01:00 UTC',
            'content' => 'replacement',
        ], $overrides);
    }
    ...
    protected function race(string $firstOperation, string $secondOperation, array $firstOverrides = [], array $secondOverrides = [], string $waitTable = 'assessment_attempts'): array
    {
        ...
        try {
            ...
            $second = $this->startWorker($secondOperation, $this->arguments(overrides: $secondOverrides + [
                'observed_at' => '2026-09-17 09:02:00 UTC',
            ...
            if ($secondOperation === 'file') {
                ...
                Storage::disk('blitz_submit_concurrency')->assertExists($storedBlob['key']);
            }
        } finally {
    ...
    protected function assertSubmitSuccess(array $result, string $student = 'first'): void
    {
        ...
        $this->assertSame('Blitz attempt submitted successfully.', $result['body']['message']);
        ...
        $this->assertLessThan($attempt['deadline_at'], $attempt['submitted_at']);
        $this->assertSame('2026-09-17 09:10:00+00', $attempt['deadline_at']);
    ...
    {
        $this->assertSame($this->submitLocks(), array_slice($result['locks'], 0, 4));
        $this->assertNotContains('topic_result_pairs', array_column($result['locks'], 'table'));
        $this->assertNotEmpty($result['gate_reads']);
        foreach ($result['gate_reads'] as $read) {
            $this->assertSame(1, $read['transaction_level']);
            $this->assertSame($this->submitLocks(), array_slice($read['locks'], 0, 4));
        }
    }
    ...
    {
        $path = tempnam(sys_get_temp_dir(), 's08_be_008_race_');
...
config([
    'filesystems.private_files_disk' => 'blitz_submit_concurrency',
    'filesystems.disks.blitz_submit_concurrency' => [
...
if ($mode === 'setup') {
    ...
    foreach (['first', 'second'] as $name) {
        ...
        $attempt = AssessmentAttempt::factory()->create([
            ...
            'started_at' => now(), 'deadline_at' => Carbon::parse('2026-09-17 09:10:00 UTC'),
        ]);
        ...
        $file = File::factory()->studentSubmission()->create([
            ...
            'original_name' => 'initial-'.$name.'.pdf', 'storage_disk' => 'blitz_submit_concurrency',
        ...
        Storage::disk('blitz_submit_concurrency')->put($key, $bytes);
...
if ($mode === 'cleanup') {
    ...
    $prefix = 'blitz-submit-concurrency-';
    ...
    DB::transaction(function () use ($arguments): void {
        foreach ([IdempotencyRecord::class, AnswerTextValue::class, AnswerFile::class, AttemptAnswer::class,
            ...
            BlitzTask::class, Assessment::class, Topic::class, GroupTeacherMembership::class,
...
function snapshot(array $arguments): array
{
    ...
    return [
        ...
        'blitz_status' => BlitzTask::query()->whereKey($arguments['assessment'])->firstOrFail()->status->value,
        'recipients' => AssessmentStudent::query()->where('assessment_id', $arguments['assessment'])
...
Event::listen(TransactionRolledBack::class, function () use (&$commits): void {
    $commits[] = DB::transactionLevel();
});
DB::connection()->beforeExecuting(function ($sql) use (&$deadlineEntries, &$commits, $arguments): void {
    ...
    foreach (debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS) as $frame) {
        if (($frame['class'] ?? null) === FinalizeTimedOutBlitzAttempts::class
            && ($frame['function'] ?? null) === '__invoke') {
            $entry = ['transaction_level' => DB::transactionLevel(), 'preceding_transaction_ends' => $commits,
                'claims' => IdempotencyRecord::query()->where('institution_id', $arguments['institution'])
                    ->where('user_id', $arguments['student'])->count()];
...
DB::listen(function ($query) use (&$locks, &$gateReads, &$held, &$attemptWrites, $mode, $arguments, $pid): void {
    if (str_starts_with($query->sql, 'update "assessment_attempts"') && in_array($arguments['attempt'], $query->bindings, true)) {
        $attemptWrites++;
    }
    ...
    if (str_starts_with($query->sql, 'select') && str_contains($query->sql, 'from "attempt_answers"')) {
        ...
        if (in_array(StudentHomeworkAttemptAnswerStates::class, $classes, true)
            && in_array(SubmitStudentBlitzAttempt::class, $classes, true)) {
...
try {
    $result = ['pid' => $pid];
    if ($mode === 'deadline') {
        $result['finalized_count'] = app(FinalizeTimedOutBlitzAttempts::class)($arguments['institution'], $arguments['assessment']);
    } elseif ($mode === 'close') {
        app(CloseTeacherBlitz::class)(User::query()->findOrFail($arguments['teacher']), $arguments['assessment']);
    } elseif ($mode === 'probe') {
        ...
        if ($mode === 'submit') {
            ...
            $uploadPath = tempnam(sys_get_temp_dir(), 's08_be_008_upload_');
