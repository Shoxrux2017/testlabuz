...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withExceptions(function (Exceptions $exceptions): void {
        ...
        $exceptions->render(fn (BlitzAttemptExceptionNotAllowedException $e, Request $request) => ApiErrorResponse::blitzAttemptExceptionNotAllowed($request));
        $exceptions->render(fn (BlitzAttemptExceptionAlreadyGrantedException $e, Request $request) => ApiErrorResponse::blitzAttemptExceptionAlreadyGranted($request));
        $exceptions->render(fn (BlitzNormalAttemptRequiredException $e, Request $request) => ApiErrorResponse::blitzNormalAttemptRequired($request));
        ...
        $exceptions->render(fn (TaskNotActiveException $e, Request $request) => ApiErrorResponse::taskNotActive($request));
        ...
        $exceptions->render(fn (StudentHomeworkNotActiveException $e, Request $request) => ApiErrorResponse::taskNotActive($request));
        $exceptions->render(fn (StudentHomeworkClosedException $e, Request $request) => ApiErrorResponse::taskClosed($request));
        $exceptions->render(fn (StudentHomeworkArchivedException $e, Request $request) => ApiErrorResponse::taskArchived($request));
        $exceptions->render(fn (StudentHomeworkDeadlinePassedException $e, Request $request) => ApiErrorResponse::deadlinePassed($request));
        $exceptions->render(fn (StudentAssessmentNotAssignedException $e, Request $request) => ApiErrorResponse::studentAssessmentNotAssigned($request));
        $exceptions->render(fn (StudentBlitzAssessmentNotAssignedException $e, Request $request) => ApiErrorResponse::studentBlitzAssessmentNotAssigned($request));
        $exceptions->render(fn (StudentBlitzAttemptNotEditableException $e, Request $request) => ApiErrorResponse::studentBlitzAttemptNotEditable($request));
        $exceptions->render(fn (StudentBlitzAttemptsExhaustedException $e, Request $request) => ApiErrorResponse::studentBlitzAttemptsExhausted($request));
        $exceptions->render(fn (StudentBlitzConflictException $e, Request $request) => ApiErrorResponse::businessConflict($request));
        $exceptions->render(fn (StudentBlitzNotActiveException $e, Request $request) => ApiErrorResponse::studentBlitzNotActive($request));
        $exceptions->render(fn (StudentBlitzTimeExpiredException $e, Request $request) => ApiErrorResponse::studentBlitzTimeExpired($request));
        $exceptions->render(fn (AttemptsExhaustedException $e, Request $request) => ApiErrorResponse::attemptsExhausted($request));
        $exceptions->render(fn (IdempotencyKeyReusedException $e, Request $request) => ApiErrorResponse::idempotencyKeyReused($request));
        $exceptions->render(fn (InstitutionSettingsIncompleteException $e, Request $request) => ApiErrorResponse::institutionSettingsIncomplete($e->missingFields, $request));
        $exceptions->render(fn (OfficialCohortMismatchException $e, Request $request) => ApiErrorResponse::officialCohortMismatch($request));
        $exceptions->render(fn (StudentHomeworkConflictException $e, Request $request) => ApiErrorResponse::businessConflict($request));
        $exceptions->render(fn (AttemptNotEditableException $e, Request $request) => ApiErrorResponse::attemptNotEditable($request));
