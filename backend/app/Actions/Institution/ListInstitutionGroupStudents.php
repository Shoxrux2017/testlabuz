...
class ListInstitutionGroupStudents
{
    ...
    public function __invoke(
        ...
        $query = User::query()
            ->select([
                ...
                'users.phone',
                'users.is_active',
                'group_student_memberships.started_at',
            ])
            ->join('group_student_memberships', 'group_student_memberships.student_id', '=', 'users.id')
            ->where('users.institution_id', $actor->institution_id)
            ->where('users.role', UserRole::Student->value)
            ->where('group_student_memberships.institution_id', $actor->institution_id)
            ->where('group_student_memberships.group_id', $resolvedGroup->id)
    ...
    private function applySorting(Builder $query, string $sort, string $direction): void
    {
        ...
        match ($sort) {
            ...
            self::SORT_STARTED_AT => $query->orderBy('group_student_memberships.started_at', $direction),
            default => throw new InvalidArgumentException('Unsupported institution group student list sort field.'),
        };
    ...
    private function safeDirection(string $direction): string
    {
        if (! in_array($direction, self::allowedDirections(), true)) {
            throw new InvalidArgumentException('Unsupported institution group student list sort direction.');
        }
