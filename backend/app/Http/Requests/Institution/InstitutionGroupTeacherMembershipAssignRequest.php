...
class InstitutionGroupTeacherMembershipAssignRequest extends FormRequest
{
    private const ACCEPTED_BODY_KEY = 'teacher_ids';
    ...
    public function withValidator(Validator $validator): void
    {
        ...
        $teacherIds = $validationData[self::ACCEPTED_BODY_KEY] ?? null;
        $queryKeys = array_keys($this->query->all());

        $validator->after(function (Validator $validator) use ($hasJsonObjectBody, $unknownBodyKeys, $teacherIds, $queryKeys): void {
            if (! $hasJsonObjectBody) {
                $validator->errors()->add('body', 'A valid application/json object body is required.');
            }

            if ($this->containsCanonicalDuplicate($teacherIds)) {
                $validator->errors()->add(self::ACCEPTED_BODY_KEY, 'The teacher ids must not contain duplicates.');
            }
    ...
    public function teacherIds(): array
    {
        ...
        $teacherIds = $this->validated(self::ACCEPTED_BODY_KEY);
    ...
    private function containsCanonicalDuplicate(mixed $teacherIds): bool
    {
        ...
        $canonicalIds = array_map(
            static fn (mixed $teacherId): mixed => is_string($teacherId) ? strtolower($teacherId) : $teacherId,
            $teacherIds,
        );
