...
class InstitutionGroupCreateRequest extends FormRequest
{
    ...
    {
        return [
            'name' => ['required', 'string', 'min:1', 'max:'.self::NAME_MAX_LENGTH],
    ...
    /**
     * @return array{name: string, level: ?string, subject_direction: ?string, description: ?string}
     */
    public function groupAttributes(): array
    {
        return [
            'name' => (string) $this->validated('name'),
            'level' => $this->nullableString('level'),
            'subject_direction' => $this->nullableString('subject_direction'),
