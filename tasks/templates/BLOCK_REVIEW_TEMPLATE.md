...
Project Owner or approved CI executes the required checkpoint verification
commands and returns the observed evidence.

Codex must not be used merely to run the full suite, build, static checks, or
collect Phase 2 evidence. Codex is used only if ChatGPT identifies a focused
implementation finding that requires code changes. Codex must not make missing
product, architecture, API, database, security, tenant, lifecycle, concurrency,
...
Project Owner/CI does not need to read those documents merely to execute the
approved commands.

Codex must not reopen them to reinterpret implementation contracts. If a
focused fix is later required, Codex receives only the focused fix contract,
applicable `AGENTS.md`, and directly relevant source/tests.
...
The initial Phase 2 checkpoint runs the complete checkpoint verification planned
...
Approved CI may execute the same commands when available.

Codex is not the default executor for Phase 2.

Record exact commands and exact observed results. Do not report a command as
passing unless it was actually run and passed.
...
Record unavailable tooling, environment blockers, and pre-existing failures
separately from candidate implementation defects.

---

## 8A. Evidence Validity After Findings and Focused Fixes

The initial Phase 2 checkpoint requires the complete planned evidence in
Section 8.

If the checkpoint identifies a finding and a later focused fix is delivered,
do not automatically rerun every previously passing broad check.

ChatGPT decides which prior PASS evidence remains valid and which verification
surface was materially invalidated by the fix.

Use these default rules:

| Later change | Checkpoint evidence effect |
|---|---|
...
| Narrow production fix in one feature | Preserve unrelated PASS evidence; rerun focused affected checks and any materially affected checkpoint/integration path |
| Shared auth/session/router/client/middleware/error infrastructure | May invalidate broader checkpoint evidence; ChatGPT decides exact rerun |
| Public API/schema/migration/authorization/tenant/security change | Normally invalidates the corresponding checkpoint/security/integration surface |
| Dependency/platform/build-system change | Normally invalidates relevant static/build/checkpoint evidence |
| Required command previously failed | That command must eventually pass after the correction |

Record the validity decision when a fix occurs:
...
2. ChatGPT prepares focused implementation contract(s) for the blocking
   finding(s);
3. Codex implements only the approved fix scope and runs only its focused
   task-level verification;
4. the Project Owner performs the approved Git/GitHub delivery by default;
5. ChatGPT applies Section 8A to determine which checkpoint evidence remains
   valid;
6. Project Owner/CI reruns only the materially invalidated checkpoint checks,
   while every previously failing required command must eventually pass;
7. ChatGPT assigns the new Phase 2 verdict.

Do not fix findings inside this read-only review.
