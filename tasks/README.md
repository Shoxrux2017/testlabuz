...
| Stage 3 — Institution Administration and User Management | `In Progress` | See `STAGE_03_TASK_INDEX.md`; `S03-INT-001`, `S03-BE-001`, `S03-BE-002`, and `S03-BE-003` are `Accepted / PASS / Delivered`; next implementation gate is `S03-BE-004` |
...
Stage 3 is `In Progress` with `S03-INT-001`, `S03-BE-001`, `S03-BE-002`, and
`S03-BE-003` accepted and delivered. The Institution Admin dashboard,
own-profile, and Institution User list/detail APIs are delivered; the next
implementation gate is `S03-BE-004`. Stage 3 is not closed, and Stage 4 has not
started and remains blocked until the separate Stage 3 Closure Review is
